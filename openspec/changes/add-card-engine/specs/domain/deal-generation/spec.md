...
of the shuffled deck drawn first. The waste and all four foundations SHALL start empty.
Input-agnostic: triggered by starting a game, not by a card interaction. Deterministic: the same seed
and mode always yield an identical position. *(KS-DEAL-01)*
...
one under Standard. A fresh deal SHALL be on pass 1, with no moves, no elapsed time, not started, not
won, and the starting score of its scoring rules. Input-agnostic: no interaction. Deterministic: a
mode always maps to the same draw count and scoring rules. *(KS-SET-06)*
...
- **THEN** it is on its first pass through the stock, has made no moves, has no elapsed time, is
  not yet started, is not won, and carries the starting score for its scoring rules
