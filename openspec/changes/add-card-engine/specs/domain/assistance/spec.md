...
3. the first empty column, when the group's lowest card is a King that is not already at the base of
   its column;

and otherwise SHALL report no target. This scan deliberately replaces the canonical destination
order. Input-agnostic: the shared resolution behind the smart-move tap, the double-activation
...
plan when a step is refused, or once every card of the stock and waste has been turned over since
the last card was sent — one full cycle of the talon, which may span a recycle. Finishing SHALL be
