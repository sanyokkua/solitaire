...
`cryptoSeed(source: SeedSource = globalThis.crypto)`. It throws when no source is available and never
falls back to `Math.random`. Only Phase 3's deal service calls it; nothing in the engine does. This is
...
and the sends are uncounted (D8). It returns `undefined` if a step is refused, or once every stock
and waste card has been turned over since the last send (one full talon cycle, possibly spanning a
recycle). Finish availability is exactly `finishPlan(state) !== undefined`. Phase 6 animates
...
- no source uses the identifiers `document`, `window`, `localStorage`, `sessionStorage`, `fetch` or
  `Math.random`, matched as whole identifiers;
- `crypto` appears only in `prng.ts`.
...
- `SeedSource` is `{ getRandomValues(array: Uint32Array): Uint32Array }`.
...
- **`bestTarget`** uses its own relative scan: first the column right of the source, wrapping around;
  from column 0 when the group came from the waste or a foundation.
