...
accepted command. No command SHALL change the elapsed time, seed, mode, draw count, scoring rules or
deal provenance. Input-agnostic: applies to every command. Deterministic: the same command sequence
always yields the same move count. *(KS-SCO-05, KS-STA-01)*
...
- **THEN** the elapsed time, seed, mode, draw count, scoring rules and deal provenance are
